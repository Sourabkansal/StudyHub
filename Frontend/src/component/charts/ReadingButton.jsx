...
const ReadingTimerButton = ({ bgColor, heading, data1, currentLabel }) => {
  ...
  const sendReadingTime = async () => {
    try {
      ...
      console.log('API called and data saved:', data);
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <span className="inline-block min-w-[60px] text-left">
        {!isReading ? `${heading}?` : formatTime(seconds)}
      </span>
    </button> 
  );
};

export default ReadingTimerButton;
